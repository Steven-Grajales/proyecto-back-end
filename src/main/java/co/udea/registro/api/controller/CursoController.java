...
public class CursoController {
    ...
    @GetMapping("/{id}")
    @ApiOperation(value = "Buscar curso", response = Page.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "El curso fue encontrado", response = Page.class),
            @ApiResponse(code = 404, message = "No se encontró el curso"),
            @ApiResponse(code = 500, message = "Error interno al procesar la respuesta")})
    public ResponseEntity<CursoWrapper> consultarCurso(@PathVariable String id){
        log.info("RESTapi: Buscar curso");
        return ResponseEntity.ok(cursoService.consultarCurso(id));
    }
